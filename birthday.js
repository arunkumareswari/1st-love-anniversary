...
const puzzleLayer   = $('puzzleLayer');
const puzzleClose   = $('puzzleClose');
const portalCurtain = $('portalCurtain');
const tokens        = [$('token1'), $('token2'), $('token3')];

// Handle bfcache: if user navigates back to this page, remove the loading screen
window.addEventListener('pageshow', (e) => {
  if (e.persisted && portalCurtain) {
    portalCurtain.classList.remove('is-active');
  }
});
...
function buildSprites(){
  ...
  BOKEH = [makeBokeh('188,230,255'), makeBokeh('196,255,244'), makeBokeh('210,238,255')];
  SPARKLE = makeSparkle();
}
...
function barkGrad(x1, y1, x2, y2, depth){
  ...
  g.addColorStop(0, `hsl(210 26% ${20 + depth * 3}%)`);
  g.addColorStop(1, `hsl(208 24% ${35 + depth * 5}%)`);
  return g;
}
...
function buildScene(){
  ...
  bgGrad.addColorStop(0.46, '#b3e5fc');
  bgGrad.addColorStop(0.78, '#81d4fa');
  bgGrad.addColorStop(1, '#4fc3f7');
  glowGrad = ctx.createRadialGradient(cx, cy, ry * 0.1, cx, cy, ry * 1.55);
  glowGrad.addColorStop(0, 'rgba(170,230,255,0.6)');
  glowGrad.addColorStop(0.5, 'rgba(150,200,255,0.2)');
  glowGrad.addColorStop(1, 'rgba(150,200,255,0)');
  groundGrad = ctx.createRadialGradient(cx, H * 1.02, ry * 0.2, cx, H * 1.02, ry * 1.6);
  groundGrad.addColorStop(0, 'rgba(165,220,255,0.5)');
  groundGrad.addColorStop(1, 'rgba(165,220,255,0)');
...
function burstHearts(){
  ...
  const cols = ['#4fc3f7', '#81d4fa', '#29b6f6', '#ffd36a', '#0288d1'];
...
function armReplay(){
  ...
  if (letterBtn){
    ...
    requestAnimationFrame(() => letterBtn.classList.add('is-shown'));
  }
  if (lockBtn){
    lockBtn.hidden = false;
    requestAnimationFrame(() => lockBtn.classList.add('is-shown'));
  }
}

function openLetter(){ if (letterModal) letterModal.classList.add('is-open'); }
function closeLetter(){ if (letterModal) letterModal.classList.remove('is-open'); }

function openPuzzle(){
  if (portalCurtain) portalCurtain.classList.add('is-active');
  setTimeout(() => {
...
function resetAll(){
  ...
  if (letterBtn){ letterBtn.classList.remove('is-shown'); letterBtn.hidden = true; }
  if (lockBtn){ lockBtn.classList.remove('is-shown'); lockBtn.hidden = true; }
...
if (reduceMotion){
  ...
  if (lockBtn) lockBtn.addEventListener('click', openPuzzle);

  /* Bubu & Dudu Handlers */
  const bubuBtn     = $('bubuBtn');
  const bubuModal   = $('bubuModal');
  const bubuClose   = $('bubuClose');
  const bubuBackdrop= $('bubuBackdrop');
  const bubuHugBtn  = $('bubuHugBtn');

  function openBubuPage(){
    if (portalCurtain) portalCurtain.classList.add('is-active');
    setTimeout(() => {
      window.location.href = './play.html';
    }, 800);
  }
  function closeBubu(){ if (bubuModal) bubuModal.classList.remove('is-open'); }

  if (bubuBtn) bubuBtn.addEventListener('click', openBubuPage);
  if (bubuClose) bubuClose.addEventListener('click', closeBubu);
  if (bubuBackdrop) bubuBackdrop.addEventListener('click', closeBubu);

  if (bubuHugBtn) {
    bubuHugBtn.addEventListener('click', (e) => {
      const rect = bubuHugBtn.getBoundingClientRect();
      const cx = rect.left + rect.width / 2;
      const cy = rect.top + rect.height / 2;
      for (let i = 0; i < 20; i++) {
        setTimeout(() => spawnTouchSparkles(cx + rand(-50, 50), cy + rand(-20, 20)), i * 40);
      }
      bubuHugBtn.textContent = 'Hug Sent! 🐻💖✨';
      setTimeout(() => { bubuHugBtn.textContent = 'Send Bubu & Dudu Hug 🐻💖'; }, 2500);
    });
  }
}

/* --- Interactive Touch Sparkles & Petals Burst --- */
function spawnTouchSparkles(x, y){
  const colors = ['#4fc3f7', '#80deea', '#b3e5fc', '#ffffff', '#ffd36a', '#ff99b4'];
  for (let i = 0; i < 7; i++){
    const el = document.createElement('span');
    el.className = 'touch-sparkle';
    const isHeart = i < 3;
    const size = isHeart ? rand(10, 18) : rand(4, 8);
    el.style.cssText = `left:${x}px;top:${y}px;width:${size}px;height:${size}px;margin:-${size/2}px 0 0 -${size/2}px;`;
    if (isHeart) {
      ...
      el.style.background = `radial-gradient(circle, ${pick(colors)}, transparent 70%)`;
    }
    document.body.appendChild(el);
    const angle = rand(0, Math.PI * 2);
    const dist = rand(25, 85);
    gsap.to(el, {
      x: Math.cos(angle) * dist,
      y: Math.sin(angle) * dist - rand(15, 40),
      rotation: rand(-60, 60),
      scale: rand(0.5, 1.2),
      duration: rand(0.6, 1.0),
      ease: 'power2.out',
    });
    gsap.to(el, { opacity: 0, duration: 0.4, delay: rand(0.3, 0.5), ease: 'power1.in', onComplete: () => el.remove() });
  }
}

window.addEventListener('pointerdown', (e) => {
  if (e.target.closest('#archery') || e.target.closest('#letterModal') || e.target.closest('#bubuModal') || e.target.closest('#bubuBtn') || e.target.closest('#replay') || e.target.closest('#letterBtn') || e.target.closest('#lockBtn')) return;
  spawnTouchSparkles(e.clientX, e.clientY);
});
